...
public class MoveUpAndDownSporadically : MonoBehaviour
{
	...
	void FixedUpdate()
    {
		...
		transform.position = direction == Direction.up ? 
			Vector2.SmoothDamp(transform.position, topPos, ref velocity, .3f) :
			Vector2.SmoothDamp(transform.position, bottomPos, ref velocity, .3f);
